...
class LogFileHandler(FileSystemEventHandler):
    ...
    def _parse_log_line(self, line: str, line_number: int) -> Optional[log_streaming_service_pb2.LogStreamResponse]:
        """Parse a log line into a LogStreamResponse."""
        try:
            # This matches the parser pattern from the existing codebase
            import re
            pattern = re.compile(
            ...
            if match:
                timestamp, component, subcomponent, pid, thread_id, level, \
                    process_id, process_name, entry_type, message = match.groups()
            ...
            self.logger.warning(f"Failed to parse log line {line_number}: {e}")
            return None
                ...
                for line in f:
                    if line.strip():
                        ...
                        if not filter_patterns or any(pattern.lower() in line.lower() 
                            ...
                            handler = LogFileHandler(log_file_path, stream_callback, 
                                                   filter_patterns, self.logger)
                            entry = handler._parse_log_line(line, line_number)
                            if entry:
